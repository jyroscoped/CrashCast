...
def _report_form_context(db: Session) -> dict:
    ...
    return {
        "reporters": reporters,
        "behavior_categories": [item.value for item in BehaviorCategory],
    }
...
def _dashboard_context(db: Session) -> dict:
    ...
    users = (
        db.execute(select(Users).order_by(desc(Users.created_at)).limit(settings.admin_page_size))
        .scalars()
        .all()
    )
    profiles = (
...
def report_form(request: Request, db: Session = Depends(get_db)):
    context = {"request": request, **_report_form_context(db)}
...
async def submit_report_form(
    ...
    try:
        ...
        if media_file and media_file.filename:
            ...
            target_path = upload_dir / local_name
...
def admin_dashboard(
    ...
    context = {"request": request, **_dashboard_context(db)}
